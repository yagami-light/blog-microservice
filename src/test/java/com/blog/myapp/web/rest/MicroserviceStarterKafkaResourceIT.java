...
import com.blog.myapp.config.KafkaProperties;
...
class MicroserviceStarterKafkaResourceIT {
    ...
    void setup() {
        ...
        MicroserviceStarterKafkaResource kafkaResource = new MicroserviceStarterKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/microservice-starter-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/microservice-starter-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
